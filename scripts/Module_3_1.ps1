...
$INVENTORY_SOURCE=Add-PPDMinventory_sources -Hostname $VCENTER_HOST -port 443 -Type VCENTER -isHostingvCenter -ID $CREDS.id -Name "DEMO VCENTER"
$DISCOVERY=$INVENTORY_SOURCE | Start-PPDMdiscoveries -level DataCopies -start inventory-sources 
Write-Host $Discovery.name
do { 
    Sleep 5
